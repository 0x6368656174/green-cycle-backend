...
export class DashboardChartComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('chart')
  chartContainer: ElementRef<HTMLCanvasElement>;
  ...
  ngOnInit(): void {
    this.range$ = combineLatest(this.startTime$, this.endTime$).pipe(
      map(
        ([startTime, endTime]) => `${startTime.format('DD MMMM')} - ${endTime.format('DD MMMM')}`,
      ),
    );
      ...
      switchMap(days => {
        const values$ = days.map(day => {
          return this.firestore
            ...
            .valueChanges()
            .pipe(map(value => (value ? value[this.statistic] || 0 : 0)));
        });
